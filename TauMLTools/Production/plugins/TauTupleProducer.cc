...
namespace {
  ...
  getTime_subleadCand(const tau_analysis::ObjPtr<const pat::Tau>& tau)
  {
    static constexpr float default_value = tau_tuple::DefaultFillValue<float>();
    if ( !tau ) return std::pair<float, float>(default_value, default_value);
    int count(0);
    float firstsubleadCand_time(default_value);
    float secondsubleadCand_time(default_value);
    reco::CandidatePtrVector signalCands = tau->signalCands();
    for (auto sig : signalCands)
    {
      if (sig->pdgId() == 22) continue;
      count += 1;
      if ( count == 1 ) continue;
      auto packedCand = dynamic_cast<const pat::PackedCandidate*>(sig.get());
      if ( count == 2 && packedCand->timeError() != -1 ) firstsubleadCand_time = packedCand->time();
      else if ( count == 3 && packedCand->timeError() != -1 ) secondsubleadCand_time = packedCand->time();
      else break;
    }
    return std::pair<float, float>(firstsubleadCand_time, secondsubleadCand_time);
  }
  float
  calculate_avgtime(const reco::Vertex recVertex, const pat::PackedCandidateCollection& cands)
  {
    float sum(0);
    float count(0);
    for(auto cand : cands)
    {
      if( !(cand.pt() > 2 && fabs(cand.eta()) < 2.4 && cand.charge()) ) continue;
      if (cand.timeError() == -1) continue;
      if ( fabs(cand.vertex().z() - recVertex.z()) < 0.1 )
      {
        float pt = cand.pt();
        sum += pt * cand.time();
        count += pt;
  ...
  reco::Vertex
  find(const math::XYZPointF* genvertex, const reco::VertexCollection* recvertex)
  {
    float maxsumpt2(0);
    reco::Vertex maxsumpt2vertex;
    for( auto & vertex : *recvertex )
    {
      float sumpt2(0);
      if( fabs(vertex.z() - genvertex->z()) < 0.1 )
      {
        for (reco::Vertex::trackRef_iterator it = vertex.tracks_begin(); it != vertex.tracks_end(); it++) sumpt2 += (**it).pt()*(**it).pt();
      }
      if( sumpt2 > maxsumpt2 )
      {
        maxsumpt2vertex = vertex;
        maxsumpt2 = sumpt2;
      }
    }
    return maxsumpt2vertex;
  }
} // anonymous namespace
...
public:
    TauTupleProducer(const edm::ParameterSet& cfg) :
        ...
        genVertex_token(mayConsume<math::XYZPointF>(cfg.getParameter<edm::InputTag>("genvertex"))),
...
private:
	...
	auto genMETs = getHandle(event, genMETs_token);
        ...
        auto genVertex = getProduct(event, genVertex_token, isMC);
        auto recVertex = getProduct(event, vertices_token, isMC);
        auto genJets = getProduct(event, genJets_token, isMC);
        auto genJetFlavourInfos = getProduct(event, genJetFlavourInfos_token, isMC);

        const reco::Vertex genmatchedrecVertex = find(genVertex, recVertex);
        float avgtime = calculate_avgtime(genmatchedrecVertex, *cands);
	...
	tauTuple().tauSpinnerWTEven = (*tauSpinnerWTEven);
        ...
        for(size_t tauJetIndex = 0; tauJetIndex < tauJets.size(); ++tauJetIndex) {
            ...
            FillTau(tauJet.tau, "tau_", avgtime);
            FillTau(tauJet.boostedTau, "boostedTau_", avgtime);
            FillJet(tauJet.jet, "jet_");
            FillJet(tauJet.fatJet, "fatJet_");

            FillPFCandidates(tauJet.cands, "pfCand_", avgtime);
            FillPFCandidates(tauJet.lostTracks, "lostTrack_", avgtime);
...
private:
  ...
  void FillTau(const ObjPtr<const pat::Tau>& tau, const std::string& prefix, const float& avgtime)
    {
        if(tau) {
        ...
        tauTuple.get<float>(prefix + "leadChargedHadronCand_time_wrt_avgtime") = (leadChargedHadrCand && leadChargedHadrCand->timeError() != -1) ? (leadChargedHadrCand->time() - avgtime) : default_value;
        std::pair<float, float> time_subleadCands = getTime_subleadCand(tau);
        tauTuple.get<float>(prefix + "firstsubleadChargedHadronCand_time_wrt_avgtime") = (time_subleadCands.first != default_value) ? (time_subleadCands.first - avgtime) : default_value;
        tauTuple.get<float>(prefix + "secondsubleadChargedHadronCand_time_wrt_avgtime") = (time_subleadCands.second != default_value) ? (time_subleadCands.second - avgtime) : default_value;
  ...
  void FillPFCandidates(const TauJet::PFCandCollection& cands, const std::string& prefix, const float& avgtime)
    {
        ...
        for(const PFCandDesc& cand_desc : cands) {
            ...
            push_back("time_wrt_avgtime", cand->timeError() != -1 ? cand->time() - avgtime : default_value);
	...
	auto push_back = [&](const std::string& prefix, const std::string& name, float value) {
        ...
        auto fillShape = [&](const reco::Photon::ShowerShape& shape, const std::string& prefix) {
            ...
            push_back(prefix, "sigmaIetaIeta", shape.sigmaIetaIeta);
            push_back(prefix, "e1x5", shape.e1x5);
            push_back(prefix, "e2x5", shape.e2x5);
            push_back(prefix, "e3x3", shape.e3x3);
            push_back(prefix, "e5x5", shape.e5x5);
            push_back(prefix, "maxEnergyXtal", shape.maxEnergyXtal);
            push_back(prefix, "hcalDepth1OverEcal", shape.hcalDepth1OverEcal);
            push_back(prefix, "hcalDepth2OverEcal", shape.hcalDepth2OverEcal);
            push_back(prefix, "hcalDepth1OverEcalBc", shape.hcalDepth1OverEcalBc);
            push_back(prefix, "hcalDepth2OverEcalBc", shape.hcalDepth2OverEcalBc);
            push_back(prefix, "effSigmaRR", shape.effSigmaRR);
            push_back(prefix, "sigmaIetaIphi", shape.sigmaIetaIphi);
            push_back(prefix, "sigmaIphiIphi", shape.sigmaIphiIphi);
            push_back(prefix, "e2nd", shape.e2nd);
            push_back(prefix, "eTop", shape.eTop);
            push_back(prefix, "eLeft", shape.eLeft);
            push_back(prefix, "eRight", shape.eRight);
            push_back(prefix, "eBottom", shape.eBottom);
            push_back(prefix, "e1x3", shape.e1x3);
            push_back(prefix, "e2x2", shape.e2x2);
            push_back(prefix, "e2x5Max", shape.e2x5Max);
            push_back(prefix, "e2x5Left", shape.e2x5Left);
            push_back(prefix, "e2x5Right", shape.e2x5Right);
            push_back(prefix, "e2x5Top", shape.e2x5Top);
            push_back(prefix, "e2x5Bottom", shape.e2x5Bottom);
...
private:
  ...
  edm::EDGetTokenT<math::XYZPointF> genVertex_token;
